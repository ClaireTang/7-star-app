<template>
	<view class="point-detail">
		...
		<view class="goods-desc">
			...
			<view class="cell">
				<view class="cell-text flex">
					<view class="price" style="text-align: left;">
						<text class="big" style="font-weight: bold;font-size: 0.8rem">{{ goodsInfo.point || '0' }}</text>
						<text style="font-weight: bold;font-size: 0.6rem">积分</text>
						<text class="old">原价:{{ product.mktprice || '0.00' }}元</text>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="footer btn-square btn-b sign-btn" @click="toshow(2)">
			<view id="openActionSheet" class="btn">
				立即兑换  
			</view>
		</view>
				...
				<view class="pop-c">
					<view class="pop-t">
						...
						<view class='goods-information'>
							...
							<view class='pop-goods-price red-price'>{{ goodsInfo.point || ''}}积分</view>
						</view>
				...
				if (this.buyNum > 0) {
					...
					this.$api.addCart(data, res => {
						if (res.status) {
							...
							this.$common.navigateTo('/pages/point/place-order/index?cart_ids=' + JSON.stringify(cartIds));
						} else {
							this.$common.errorToShow(res.msg);
						}
					}, res => {
...
<style>
	...
	.goods-title .name {
	    position: relative;
	    padding: 28upx 0 28upx;
		border-bottom: 0.5upx solid #F8F8F8;
	}	
	.goods-desc .name .title{
		color: #7c7c7c;
		font-size: 28upx;
		margin-left:6upx;
	}
	...
	.goods-desc .cell .price {
	    color: #f24749;
	    font-size: 0.65rem;
	}
	.goods-desc .cell .price .old {
	    text-decoration: line-through;
	    color: #999;
	}
	.goods-desc .cell .flex span {
	    width: 100%;
	    text-align: center;
	}
